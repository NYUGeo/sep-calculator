...
class figure5 (object):
    ...
    def z(self):
        ...
        """
        return self.zw * (np.cos(self.beta-self.omega))/(np.cos(self.beta)*np.cos(self.omega))
    ...
    def Ja(self):
        ...
        """
        J_part1 = ((self.gamma*self.z()*np.cos(self.beta)*np.cos(self.beta+self.theta())*(1+self.kv))/np.cos(self.theta())) \
                    + self.c*np.cos(self.phi)*np.sin(self.phi)
        J_part2 = (self.gamma**2)*(self.z()**2)*((np.cos(self.beta))**2)*((1+self.kv)**2) \
                    * ((((np.cos(self.beta+self.theta()))**2)-((np.cos(self.phi))**2))/((np.cos(self.theta()))**2))
        J_part3 = (self.c**2)*((np.cos(self.phi))**2)
        J_part4 = (2*self.c*self.gamma*self.z()*np.cos(self.phi)*np.sin(self.phi)*np.cos(self.beta) \
                    * np.cos(self.beta+self.theta())*(1+self.kv))/(np.cos(self.theta()))
        
        return (1/((np.cos(self.phi))**2))*(J_part1 - np.sqrt(J_part2+J_part3+J_part4))

    def alpha_a(self, degrees=False):
        """
        Equation 18: Obliquity
        """
        alpha_a_p1 = (((2*np.cos(self.theta())*np.cos(self.beta+self.theta()))/(np.cos(self.beta)*(1+self.kv)))*(self.Ja() \
                        /(self.gamma*self.z()))-1)
        ...
        alpha_a = np.arctan(((alpha_a_p1*alpha_a_p2)+alpha_a_p3) \
                    /(2*(alpha_a_p1*((np.cos(self.beta-self.omega))**2)+((np.sin(self.theta()+self.omega))**2))))
    ...
    def Ka(self):
        ...
        """
        return ((np.cos(self.beta)*(1+self.kv)*(np.sin(self.theta()+self.omega)**2 - np.cos(self.beta-self.omega)**2)) \
               /(np.cos(self.alpha_a())*np.cos(self.beta+self.theta())*np.cos(self.theta()))
               ) + (
               ((2*(self.Ja()/(self.gamma*self.z()))*np.cos(self.beta-self.omega)**2)/np.cos(self.alpha_a())))
    ...
    def Hz(self):
        """
        Equation 21: Vertical distance between the heel of the retaining wall and its backfill slope surface
        """
        return self.H * (np.cos(self.beta-self.omega))/(np.cos(self.beta)*np.cos(self.omega))
...
### Test with:
### test = figure5(0.2, -0.1, 20, 15, 30, 23, 20, 15, 6)
